...
export const inputParamsSchema = {
  "id": z.number().int().describe("A unique integer value identifying this ASN range."),
  "name": z.string().min(1).max(100).optional(),
  "slug": z.string().regex(new RegExp("^[-a-zA-Z0-9_]+$")).min(1).max(100).optional(),
  "rir": z.number().int().optional(),
  "start": z.number().int().gte(1).lte(4294967295).optional(),
  "end": z.number().int().gte(1).lte(4294967295).optional(),
