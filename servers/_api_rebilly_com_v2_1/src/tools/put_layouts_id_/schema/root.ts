...
export const inputParamsSchema = {
  ...
  "name": z.string().describe("The name of the layout string"),
  "items": z.array(z.object({ "planId": z.string(), "starred": z.boolean().describe("Boolean if the plan should be starred (special callout presentation)").optional(), "order": z.number().int().describe("Item's order in Layout").readonly().optional(), "_links": z.array(z.string()).min(1).max(1).describe("The links related to resource").readonly().optional() })).describe("The array of layout items (planId and starred)").optional(),
