...
```json
{
  "mcpServers": {
    "portkey-ai": {
      ...
      "url": "https://mcp.open-mcp.org/api/server/portkey-ai@latest/mcp"
    }
  }
}
```
...
```ini
https://mcp.open-mcp.org/api/server/portkey-ai@latest/mcp?FORWARD_VAR_OPEN_MCP_BASE_URL=https%3A%2F%2Fapi.example.com
```
...
- `X_PORTKEY_API_KEY` - gets sent to the API provider
- `API_KEY` - gets sent to the API provider
- `X_PORTKEY_CONFIG` - gets sent to the API provider
- `X_PORTKEY_CUSTOM_HOST` - gets sent to the API provider
- `X_PORTKEY_PROVIDER` - gets sent to the API provider
- `X_PORTKEY_VIRTUAL_KEY` - gets sent to the API provider
