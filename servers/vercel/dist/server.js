...
function stringify({ value, arrayToCSV, }) {
...
async function registerToolFromOperation(operationFileRelativePath) {
    ...
    const { baseUrl, path: opPath, method, toolName, toolDescription, inputParams, security, keys, flatMap, } = operation;
    ...
    server.tool(toolName, toolDescription, inputParams, async (flat) => {
        ...
        for (const [key, value] of Object.entries(params.path || {})) {
            ...
            opPathResolved = opPathResolved.replaceAll(`{${key}}`, stringify({ value, arrayToCSV: true }));
        }
