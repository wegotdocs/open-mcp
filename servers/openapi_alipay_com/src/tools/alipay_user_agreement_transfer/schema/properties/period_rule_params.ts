...
export const inputParams = {
  ...
  "scene_rule_params": z.record(z.any()).describe("<llm-instruction>This part of the input schema is truncated. If you want to pass the property `scene_rule_params` to the tool, first call the tool `expandSchema` with \"/properties/period_rule_params/properties/scene_rule_params\" in the list of pointers. This will return the expanded input schema which you can then use in the tool call. You may have to call `expandSchema` multiple times if the schema is nested.</llm-instruction>").optional(),
